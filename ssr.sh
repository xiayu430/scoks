...
#	Modified: Hide dependency output, show progress
#=================================================
...
generate_random_port() {
    local ports_to_avoid=(22 80 443 8080 53 21 25 110 143 465 587 993 995 3306 3389)
    while true; do
        port=$(shuf -i 10000-65535 -n 1)
        if ! printf '%s\n' "${ports_to_avoid[@]}" | grep -qx "$port"; then
            echo "$port"
            break
        fi
    done
...
generate_random_password() {
    head /dev/urandom | tr -dc A-Za-z0-9 | head -c7
}
...
# 设置配置信息
Set_config_port(){
    ssr_port=$(generate_random_port)
    echo -e "随机生成端口: ${Green_font_prefix}${ssr_port}${Font_color_suffix}"
}

Set_config_password(){
    ssr_password=$(generate_random_password)
    echo -e "随机生成密码: ${Green_font_prefix}${ssr_password}${Font_color_suffix}"
}
