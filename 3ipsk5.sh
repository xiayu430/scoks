...
# 高并发优化设置
optimize_for_high_concurrency() {
    echo "正在优化系统设置以支持高并发连接..."
...
EOF
    ...
    cat <<EOF > /etc/systemd/system.conf.d/99-high-concurrency.conf
[Manager]
...
EOF
    ...
    # 7. 临时提升当前会话的文件描述符限制
    ulimit -n 1024000 >/dev/null 2>&1 || echo "警告: 无法临时提升文件描述符限制，部分设置需重启生效"
    
    echo "高并发优化设置完成！"
    echo "注意：部分优化设置需要重启服务器才能完全生效"
}

# 启用BBR加速
enable_bbr() {
    echo "正在启用BBR加速以解决高并发问题..."
    ...
    if [ "$major" -lt 4 ] || { [ "$major" -eq 4 ] && [ "$minor" -lt 9 ]; }; then
        echo "警告: 内核版本 $kernel_version 过低，BBR需要4.9+，跳过BBR设置"
        return 1
    fi
